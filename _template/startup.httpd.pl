...
my %SIZE_UNIT = ('K' => 1024, 'M' => 1024*1024, 'G' => 1024*1024*1024);
{
	...
	while(@ary) {
		...
		while($key ne '') {
			...
			# SakiaTimer
			if ($k2 eq 't0') { $key=$ky; $ENV{SakiaTimer}=0; next; }
			if ($k2 eq 't1') { $key=$ky; $ENV{SakiaTimer}=1; next; }
...
# start up
################################################################################
print "Sakia HTTP Server - Version $VERSION\n";
if ($0 =~ /\.exe$/i) {
	my $pl = $0;
	$pl =~ s/\.exe/.httpd.pl/;
...
# Exec CGI
################################################################################
sub preload_sakia_lib {
	require Sakia::Base;
	require Sakia::AutoReload;
...
sub exec_cgi {
	...
	eval {
		...
		if ($flag) {
			$Sakia::Base::RELOAD = 1;	# if Base.pm compile error, force reload
			require Sakia::Base;
			$Sakia::Base::RELOAD = 0;
