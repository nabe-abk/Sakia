use strict;
#-------------------------------------------------------------------------------
# システム標準パーサー／Sakiaパーサー
#                                             (C)2006-2019 nabe / nabe@abk
#-------------------------------------------------------------------------------
package Sakia::TextParser::Sakia;
...
sub load_plugins {
	...
	if (!$dir) {
		$INC{'Sakia/TextParser/Sakia.pm'} =~ m|(.*/)\w+\.\w+|;
		$dir=$1 . "TagPlugin/";
	}
...
sub load_tagdata {
	...
	if (!ref($data)) {
		$file = $data;
		if (!-r $file) { return; }
		$data = $ROBJ->fread_lines_cached($file);
	}
...
sub eval_load_plugin {
	...
	if ($@) { $ROBJ->error('[plugin load failed] %s', $@); }	## mskip
}
